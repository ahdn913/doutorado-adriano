...
#define l_polarizacao 0.05
#define I0 30 // número inicial de infectados #100
...
#define p_rec_u3 0.0 // probabilidade de se recuperar a partir do U	   0.05
#define p_rec_v3 0.0 // probabilidade de se recuperar a partir do V	   0.05
...
#define p_rec_v3u3 0.15 // probabilidade de re recuperar para U2, a partir de V2
...
#define p_u3su3 0.5 // probabilidade de gerar um U no contato entre S e U   0.5
#define p_u1u1v1 0.3 // probabilidade de gerar um V no contato entre U e U  0.3
#define p_u3u3v3 0.3 // probabilidade de gerar um V no contato entre U e U  0.3
...
#define p_v3su1 0.3  // probabilidade de gerar um U no contato entre V e S 0.3
#define p_v1su3 0.3  // probabilidade de gerar um U no contato entre V e S 0.3
#define p_v2su3 0.3  // probabilidade de gerar um U no contato entre V e S 0.3
#define p_v3su2 0.3  // probabilidade de gerar um U no contato entre V e S 0.3

// LEMBRETE: AJUSTAR AS PROBABILIDADES LÁ PRA BAIXO, NA PARTE DE INFECÇÃO

struct intble {
	int    s; // gênero
	int    c; // classe (morto= 0, sensíveis= 1, indignados1= 2, violentos1= 3, indignados2= 4, violentos2= 5, indignados3= 6, violentos3= 7 recuperados= 8)
...
int main(int argc, char **argv){
	int i, j, n, o, t, k, O, K, inf, i0, vio, i2, i3, cont_s, cont_i1, cont_i2, cont_i3, cont_v1, cont_v2, cont_v3, cont_r; // k é o contador de indivíduos 
	...
	double act, test, z, teste_inf, teste_i0, teste_rec, teste_inf1, teste_inf2, teste_i2, teste_i3; // test está relacionado com a mobilidade direcional, para obter um intervalo [-th, th], junto com z
	struct intble *p;
	FILE *arq;
	char clus[100];
	...
	for(i= 0; i< N; i++){					// Atribui um gênero e posição para os individuos na rede
		...
		if(I0 != i0){
			...
			teste_i3= 1.3*gsl_rng_uniform(w);
			if(teste_i3 >= 1.0){
				p[i].c= 6;
				i3++;
			}
	...
	for(i= 0; i< N; i++){
		...
		else if(p[i].c == 6 && p[i].s != 0){
			cont_i3++;
		}
		else if(p[i].c == 7 && p[i].s != 0){
			cont_v3++;
		}
	}
	sprintf(clus, "linedat/line.dat"); // Retorna uma string de acordo com a string de formatação Format
	arq= fopen(clus, "w"); // Abre o arquivo contido no primeiro parâmetro, usando o modo dado no segundo parâmetro. com o "w", o arquivo é criado se não existir.	
	fprintf(arq, "0 %d %d %d %d %d %d %d\n", cont_s, cont_i1, cont_v1, cont_i2, cont_v2, cont_i3, cont_v3);
	fclose(arq);
...
// reprodução - começo
				...
				if(p[n].c == 2 || p[n].c == 3){
					for(j= 0; j< N; j++){
						if(p[j].c == 4 || p[j].c == 5){
							if(p[j].d < l_polarizacao && p[n].s != 0 && p[j].s != 0){
								dx= p[j].x - p[n].x;
								dy= p[j].y - p[n].y;
								...
								p[n].x+= -dx + 0.6*l*cos(tht);
								if(p[n].x > L  ){ p[n].x-= L; }	 
								if(p[n].x < 0.0){ p[n].x+= L; }
								p[n].y+= -dy + 0.6*l*sin(tht);
								if(p[n].y > L  ){ p[n].y-= L; }
								if(p[n].y < 0.0){ p[n].y+= L; }
								j= N;
							}
						}
						else if(p[j].c == 6 || p[j].c == 7){
							if(p[j].d < l_polarizacao && p[n].s != 0 && p[j].s != 0){
								dx= p[j].x - p[n].x;
								dy= p[j].y - p[n].y;
								test = gsl_rng_uniform(w); 
				...
				else if(p[n].c == 6 && p[n].s != 0){ // transm ativa
					for(j= 0; j< N; j++){
						teste_inf= gsl_rng_uniform(w);
						if(l_inf > p[j].d && teste_inf < p_u3su3 && p[j].c == 1 && p[j].s != 0 && inf != 5){ // Entre U1 e S gerando U1 || REVER ESSE p_usu
							inf++;				 
							p[j].c= 6;			  
						}
						if(l_inf > p[j].d && teste_inf < p_u3u3v3 && p[j].c == 6 && p[j].s != 0 && inf != 5){ // Entre U1 e U1 gerando V1
							p[j].c= 7;
							inf++;
						}
					}
				}
				else if(p[n].c == 3 && p[n].s != 0){
					for(j= 0; j< N; j++){
						teste_inf= gsl_rng_uniform(w);
						if(teste_inf < p_v1su2){
							teste_inf1= gsl_rng_uniform(w);
							if(l_inf > p[j].d && teste_inf1 < p_v1su2 && p[j].c == 1 && p[j].s != 0 && vio != 4){
								vio++;			 
								p[j].c= 4;
							}
							else if(l_inf > p[j].d && teste_inf1 > p_v1su3 && p[j].c == 1 && p[j].s != 0 && vio != 4){
								vio++;
								p[j].c= 6;
							}
						}
				...
				else if(p[n].c == 5 && p[n].s != 0){
					for(j= 0; j< N; j++){
						teste_inf= gsl_rng_uniform(w);
						if(teste_inf < p_v2su1){
							teste_inf1= gsl_rng_uniform(w);
							if(l_inf > p[j].d && teste_inf1 < p_v2su1 && p[j].c == 1 && p[j].s != 0 && vio != 4){
								vio++;			 
								p[j].c= 2;
							}
							else if(l_inf > p[j].d && teste_inf1 > p_v2su3 && p[j].c == 1 && p[j].s != 0 && vio != 4){
								vio++;
								p[j].c= 6;
							}
						}
				...
				else if(p[n].c == 7 && p[n].s != 0){
					for(j= 0; j< N; j++){
						teste_inf= gsl_rng_uniform(w);
						if(teste_inf < p_v3su1){
							teste_inf1= gsl_rng_uniform(w);
							if(l_inf > p[j].d && teste_inf1 < p_v3su2 && p[j].c == 1 && p[j].s != 0 && vio != 4){
								vio++;			 
								p[j].c= 4;
							}
							else if(l_inf > p[j].d && teste_inf1 < p_v3su1 && p[j].c == 1 && p[j].s != 0 && vio != 4){
								vio++;
								p[j].c= 2;
							}
						}
...
// recuperação - começo
				...
				else if(p[n].c == 6 && p[n].s != 0){
					teste_rec= gsl_rng_uniform(w);
					if(teste_rec < p_rec_u3){
						p[n].c= 8;
					}
				}
				else if(p[n].c == 7 && p[n].s != 0){
					teste_rec= gsl_rng_uniform(w);
					if(teste_rec < p_rec_v3){
						p[n].c= 8;
					}
				}
				if(p[n].c == 3 && p[n].s != 0){
					...
					teste_rec= gsl_rng_uniform(w);
					if(teste_rec < p_rec_v3u3){
						p[n].c= 6;
					}
				}
				else if(p[n].c == 8 && p[n].s != 0){
					teste_rec= gsl_rng_uniform(w);
...
// morte - fim
		...
		fprintf(arq, "%d %d %d %d %d %d %d %d\n", t, cont_s, cont_i1, cont_v1, cont_i2, cont_v2, cont_i3, cont_v3);
		fclose(arq);
	}
	gsl_rng_free(w);
	free(p);
