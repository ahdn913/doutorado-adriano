// Modelo epidemiológico SIR estocástico 
// compilação: gcc sir.c -O3 -Wall -lgsl -lgslcblas -lm -o sir
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
#define l_inf 0.01 // raio de interação de infecção
#define p_inf 0.3 // probabilidade de infecção mediante contato entre S e I
#define I0 10 // número inicial de infectados
#define p_rec 0.1 // prob de se recuperar

struct intble {
	int    s; // gênero
	int    c; // classe infectiva (morto= 0, suscetível= 1, infectado= 2, recuperado= 3)
...
void op(int t, struct intble *p){
	...
	sprintf(clus, "dat/p-%d.dat", t); // Retorna uma string de acordo com a string de formatação Format
...
/*void op2(int t, struct intble *p){
	int cont_s= 0, cont_i= 0, cont_r= 0, i;
	FILE *arq;
	char clus[100];
	...
	sprintf(clus, "dat2/p-%d.dat", 1); // Retorna uma string de acordo com a string de formatação Format
	arq= fopen(clus, "w"); // Abre o arquivo contido no primeiro parâmetro, usando o modo dado no segundo parâmetro. com o "w", o arquivo é criado se não existir.	
...
int main(int argc, char **argv){
	int i, j, n, o, t, k, O, K, inf, i0, cont_s, cont_i, cont_r; // k é o contador de indivíduos 
	...
	double act, test, z, teste_inf, teste_i0, teste_rec; // test está relacionado com a mobilidade direcional, para obter um intervalo [-th, th], junto com z
	struct intble *p;
	FILE *arq;
	char clus[100];
...
// reprodução - começo
				...
				if(p[n].c == 2 && p[n].s != 0){
					for(j= 0; j< N; j++){
						...
						if(l_inf > p[j].d && teste_inf < p_inf && p[j].c == 1 && p[j].s != 0 && inf != 1){
							p[j].c= 2;
							inf++;
						}
						if(inf == 1){
