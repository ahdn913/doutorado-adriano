// gcc teste.c -O3 -Wall -lgsl -lgslcblas -lm -o teste
// ./teste
...
//#define Nx 100   // tamanho da rede em i
//#define Ny 100   // tamanho da rede em j
#define NG 500
#define gamma 0.19
#define beta 1-gamma
		...
		while(cont_inf > 0){
			while(t < NG){
				...
				for(x= 0; x< Ni*Nj; x++){
					...
					/*switch(vizinho){
						case 0:
							passivo= ((j-1+Nx)%Nx)*Nx +     i;                //top
						break;
						case 1:
							passivo= ((j-1+Nx)%Nx)*Nx +((i+1)%Nx);            //top-right
						break;
						case 2:
							passivo= j*Nx             +((i+1)%Nx);            //right
						break;
						case 3:
							passivo= ((j+1)%Nx)*Nx    +((i+1)%Nx);          	//bot-right
						break;
						case 4:
							passivo= ((j+1)%Nx)*Nx    +     i;              	//bottom
						break;
						case 5:
							passivo= ((j+1)%Nx)*Nx    +((i-1+Nx)%Nx);       	//bot-left
						break;
						case 6:
							passivo= j*Nx             +((i-1+Nx)%Nx);         //left
						break;
						case 7:
							passivo= ((j-1+Nx)%Nx)*Nx +((i-1+Nx)%Nx);         //top-left
						break;
					}*/
