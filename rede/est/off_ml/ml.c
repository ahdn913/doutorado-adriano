// Modelo epidemiológico SI estocástico (Indivíduos infectados não se recuperam)
...
#define p_rec 0.1 // probabilidade de se recuperar, virando um suscetível

struct intble {
	int    s; // gênero
	int    c; // classe infectiva (morto= 0, suscetível= 1, infectado= 2)
...
// reprodução - começo
				...
				if(p[n].c == 1 && p[n].s != 0){
					for(j= 0; j< N; j++){
						teste_inf= gsl_rng_uniform(w);
						if(0.3*l_inf > p[j].d && teste_inf < p_inf && p[j].c == 2 && p[j].s != 0){ // 0.5*l_inf é só se o infectado estiver mto perto
							p[n].c= 2;
							j= N;
						}	
					}
